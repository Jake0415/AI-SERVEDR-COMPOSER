...
export default function EquipmentTab() {
  ...
  const [editTarget, setEditTarget] = useState<EquipmentProduct | null>(null);
  ...
  // 삭제
  const [deleteTarget, setDeleteTarget] = useState<EquipmentProduct | null>(null);
  ...
  const handleAdd = async () => {
    if (!addItem || !addModel || !addMfr) return;
    ...
    try {
      const res = await fetch("/api/equipment-products", {
        ...
        body: JSON.stringify({
          equipmentCodeId: addItem, modelName: addModel, manufacturer: addMfr,
          listPrice: addListPrice, marketPrice: addMarketPrice, supplyPrice: addSupplyPrice,
        }),
      });
      if (res.ok) { setAddOpen(false); fetchProducts(); }
  ...
  const handleEdit = async () => {
    ...
    try {
      const res = await fetch(`/api/equipment-products/${editTarget.id}`, {
      ...
      if (res.ok) { setEditOpen(false); fetchProducts(); }
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/equipment-products/${deleteTarget.id}`, { method: "DELETE" });
      if (res.ok) { setDeleteOpen(false); setDeleteTarget(null); fetchProducts(); }
      else { const e = await res.json(); alert(e.error?.message ?? "삭제 실패"); }
    } catch { alert("오류"); }
  };

  // 이력
  const openHistory = async (p: EquipmentProduct) => {
    setHistoryTarget(p); setHistoryOpen(true); setHistoryLoading(true); setHistoryData([]);
    try {
      const res = await fetch(`/api/equipment-products/${p.id}/price-history?period=12`);
  ...
  return (
    ...
    <div className="space-y-4">
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-2">
                ...
                <select value={addItem} onChange={(e) => setAddItem(e.target.value)}
                  className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                  <option value="">선택</option>
                  {addItemOptions.map(n => <option key={n.id} value={n.id}>{n.name}</option>)}
                </select>
              </div>
            </div>
            <div className="space-y-2"><Label>모델명</Label><Input value={addModel} onChange={(e) => setAddModel(e.target.value)} placeholder="예: DL380 Gen11" required /></div>
            <div className="space-y-2"><Label>제조사</Label><Input value={addMfr} onChange={(e) => setAddMfr(e.target.value)} placeholder="예: HPE" required /></div>
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleAdd} disabled={addSubmitting || !addItem || !addModel || !addMfr}>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>장비 제품 수정</DialogTitle></DialogHeader>
          <div className="space-y-4">
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>장비 제품 삭제</AlertDialogTitle>
            <AlertDialogDescription>&quot;{deleteTarget?.modelName}&quot; 제품을 삭제하시겠습니까?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
