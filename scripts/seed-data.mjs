...

{
  ...
  "equipment_list": [위 예시들의 배열]
    ...
    {
      slug: "rfp-equipment-summary",
      name: "RFP 장비 목록 추출 (1차)",
      description: "RFP에서 장비 목록과 공통 요건만 추출합니다. 상세 스펙은 2차에서 추출.",
      category: "extraction",
      system_prompt: `당신은 한국 IT 인프라 RFP 분석 전문가입니다.
RFP 문서에서 장비 목록과 공통 요건만 추출하세요. 상세 스펙은 추출하지 마세요.

## 규칙
1. common_requirements에 "공통 요건/공통 사양" 섹션의 모든 내용을 빠짐없이 추출 (원문 그대로)
2. equipment_summary에는 장비명, 수량, 카테고리, 용도만 (상세 스펙은 추출하지 마세요)
3. 카테고리: x86_server, gpu_server, storage, network_switch, san_switch, security, rack, appliance, software, other
4. constraints: "불가","금지","제한","제외" 키워드 → 반드시 포함
5. notes: "GBIC","확약서","제안사 비용" 등 기타 사항 추출

## 출력 JSON
{"project_name":"프로젝트명","common_requirements":{"server_type":"","processor":"","memory_spec":"","disk_spec":"","network_base":"","raid":"","power":"","management":"","security":"","warranty_years":3,"recommended_vendors":[],"constraints":[],"notes":[]},"equipment_summary":[{"ecr_id":"ECR-002","name":"웹서버","quantity":1,"category":"x86_server","purpose":"웹 서비스 운영"}]}`,
    },
    {
      slug: "rfp-equipment-detail",
      name: "RFP 장비 상세 스펙 추출 (2차)",
      description: "특정 장비의 상세 규격을 추출합니다.",
      category: "extraction",
      system_prompt: `당신은 한국 IT 인프라 장비 규격 추출 전문가입니다.
RFP 텍스트에서 지정된 장비의 상세 규격만 정확히 추출하세요.

## 규칙
1. 해당 장비의 스펙만 추출 (다른 장비 무시)
2. RFP 원문의 수치를 정확히 사용하고 description에 원문 기재
3. 명시되지 않은 항목은 null
4. constraints/recommendations/notes 반드시 채우기

## 카테고리별 출력 JSON

서버(x86_server/gpu_server):
{"form_factor":null,"cpu":{"cores":8,"clock_ghz":2.6,"count":2,"description":"원문"},"memory":{"capacity_gb":32,"type":null,"ecc":null,"description":"원문"},"storage":[{"type":"SSD","capacity_gb":480,"interface":null,"count":2,"description":"원문"}],"network":[{"speed":"1G","type":"UTP","ports":8,"description":"원문"}],"hba":null,"gpu":null,"raid":null,"power":null,"os":null,"constraints":[],"recommendations":[],"notes":[]}

스토리지(storage):
{"capacity":{"usable_tb":40,"controller":"Active-Active","cpu_cores":24,"cache_gb":128,"drive_type":"All Flash","description":"원문"},"network":[{"speed":"32Gbps","type":"FC","ports":4}],"custom_specs":{"protocols":[],"features":[],"scale":"","replication":""},"constraints":[],"recommendations":[],"notes":[]}

네트워크(network_switch/san_switch):
{"custom_specs":{"layer":"L4","throughput":"6Gbps","concurrent_sessions":0,"memory_gb":0,"ports":[{"speed":"10G SFP+","count":2}],"features":[],"standards":[]},"power":null,"constraints":[],"recommendations":[],"notes":[]}

랙(rack):
{"custom_specs":{"rack_size":"","max_load_kg":0,"door":"","kvm":{"lcd_size":"","kvm_ports":0},"standards":[]},"constraints":[],"recommendations":[],"notes":[]}

어플라이언스(appliance):
서버 스펙 + {"custom_specs":{"장비 고유 기능 키:값"}}`,
    },
  ];
  ...
  console.log("Step 7: AI 프롬프트 4건 UPSERT 완료");

  // Step 7-1: AI 설정 (ai_settings) — API Key 암호화 저장
  {
    ...
    if (encKey && openaiKey) {
      ...
      let encrypted = cipher.update(openaiKey.replace(/^"|"$/g, ""), "utf8", "hex");
      encrypted += cipher.final("hex");
      const tag = cipher.getAuthTag();
      const combined = Buffer.concat([iv, tag, Buffer.from(encrypted, "hex")]);
      const encryptedKey = combined.toString("base64");

      await sql`INSERT INTO ai_server_composer.ai_settings (id, tenant_id, provider, openai_model, openai_api_key, updated_at)
        VALUES ('default', ${TENANT}, 'openai', 'gpt-4o', ${encryptedKey}, NOW())
        ON CONFLICT (id) DO UPDATE SET
          openai_api_key = EXCLUDED.openai_api_key,
          updated_at = NOW()`;
      console.log("Step 7-1: AI 설정 (OpenAI Key 암호화) UPSERT 완료");
    } else {
      console.log("Step 7-1: ENCRYPTION_KEY 또는 OPENAI_API_KEY 미설정 — AI 설정 건너뜀");
    }
  }
